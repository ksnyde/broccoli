...
var debug = require('debug')('broccoli:builder');
...
function loadBrocfile (options) {
	options = options || {};
  var brocName = options.brocfile || 'Brocfile.js';
  debug('Brocfile\'s name is %s', brocName);
  var brocfile = findup(brocName, {nocase: true});
  if (brocfile === null) throw new Error('Brocfile [%s] not found', brocName);
